...
import site.ycsb.db.MongoDbClient;
import site.ycsb.db.OptionsSupport;
...
public class ShardedMongo extends MongoDbClient {
  ...
  /**
   * Measure of how compressible the data is, compressibility=10 means the data
   * can compress tenfold. The default is 1, which is uncompressible
   */
  ...
  public void init() throws DBException {
    ...
    Builder mongoClientOptions = MongoClientOptions.builder().sslEnabled(true).sslInvalidHostNameAllowed(true);

    MongoClientURI uri = new MongoClientURI(url, mongoClientOptions);

    // MongoCredential cred1 =
    // MongoCredential.createCredential("CN=mongos-1-svc,OU=ExperimentServers,O=Roderick,O=BBK,L=Southminster,ST=Essex,C=UK",
    // "$external", "".toCharArray());
    MongoCredential cred1 = MongoCredential
        .createMongoX509Credential("CN=mongos-1-svc,OU=ExperimentServers,O=Roderick,O=BBK,L=Southmister,ST=Essex,C=UK");

    MongoClient mongoClient = new MongoClient(new ServerAddress("mongos-1-svc:27017"), Arrays.asList(cred1),
    ...
    try {
      ...
      // Document creds = new Document("username",
      // "CN=mongos-1-svc,OU=ExperimentServers,O=Roderick,O=BBK,L=Southminster,ST=Essex,C=UK");
      // creds.append("pwd", "");
      //
